---
title: "Server-Streaming Replies"
description: "An API field returning IAsyncEnumerable<'Item>: items delivered as they are produced, over Orleans' own async-enumerable grain extension."
---
...
split, in [Release and Production Status](/orleans-fsharp/release-status/#taskseq-is-an-upstream-boundary-when-wrapping-streaming-replies).
...
- **A durable or replayable stream.** An enumeration is a conversation with one activation. If the
  ...
  [Orleans streams](/orleans-fsharp/streaming/) or implicit subscriptions.
...
- [Functional Grain Runtime](/orleans-fsharp/functional-grains/) — the `grainContract` / `grainFor` model
- [Streaming](/orleans-fsharp/streaming/) — Orleans streams, for durable fan-out
- [Calling from C#](/orleans-fsharp/calling-from-csharp/) — `await foreach` over a facade member
- [Event Sourcing](/orleans-fsharp/event-sourcing/) — journaled definitions, which also accept `handleStream`
