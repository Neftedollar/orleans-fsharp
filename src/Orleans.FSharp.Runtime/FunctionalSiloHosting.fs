...
type internal FunctionalSiloStartupValidator(services: IServiceProvider, registry: FunctionalGrainRegistry) =
    ...
    let validate () =
        ...
        let persistenceOptions =
            services.GetRequiredService<IOptions<FunctionalPersistenceOptions>>().Value

        if obj.ReferenceEquals(persistenceOptions.DefaultStateCodec, null) then
            fail SiloStage "the silo-wide functional persistent-state codec cannot be null."

        if obj.ReferenceEquals(persistenceOptions.DefaultJournalCodec, null) then
            fail SiloStage "the silo-wide functional journal codec cannot be null."
        ...
        // Every hosted argument and reply type needs the Orleans serializer. Durable application
        // types need it only when their effective codec is Orleans binary: JSON-backed state and
        // journal cells expose generated byte envelopes to Orleans instead. Resolve the complete
        // element > grain > silo chain here, before the silo admits traffic.
        for entry in snapshot do
            ...
            let binaryFacets =
                definition.Facets
                |> Array.choose (fun facet ->
                    let selected =
                        defaultArg facet.CodecOverride persistenceOptions.DefaultStateCodec
                        |> FunctionalPersistenceEncoding.ensure "DefaultStateCodec"

                    match selected.Kind with
                    | FunctionalPersistenceCodecKind.FSharpJson -> None
                    | FunctionalPersistenceCodecKind.OrleansBinary ->
                        ...
                        | Some reason ->
                            fail
                                StartupStage
                                $"the stored type '{descriptor.StoredType.FullName}' of persistent state '{descriptor.StateName}' (provider '{descriptor.ProviderName}') attached to grain type '{definition.GrainTypeName}' cannot be held in a direct Orleans IPersistentState with codec '{selected.Id}': {reason}. Select FunctionalPersistenceCodec.FSharpJson for this state or grain, or configure it as the silo default."
                        | None -> Some(descriptor.StateName, descriptor.StoredType))

            SerializerPreflight.ensureStoredTypes provider definition.GrainTypeName binaryFacets
            ...
            | Some journal ->
                let selected =
                    defaultArg journal.CodecOverride persistenceOptions.DefaultJournalCodec
                    |> FunctionalPersistenceEncoding.ensure "DefaultJournalCodec"

                if selected.Kind = FunctionalPersistenceCodecKind.OrleansBinary then
...
module internal FunctionalSiloServices =
    ...
    let private registryOf (services: IServiceCollection) =
        ...
        | None ->
            ...
            services.AddOptions<FunctionalPersistenceOptions>() |> ignore
...
type FunctionalGrainSiloHostingExtensions =
    ...
    /// <summary>
    /// Configure silo-wide durable payload defaults. A grain-level selection overrides these
    /// values, and <c>PersistentState.withCodec</c> overrides both for one state element.
    /// </summary>
    [<Extension>]
    static member ConfigureFunctionalPersistence
        (builder: ISiloBuilder, configure: Action<FunctionalPersistenceOptions>)
        : ISiloBuilder =
        if isNull (box builder) then
            fail SiloStage "ConfigureFunctionalPersistence requires a silo builder."

        if isNull configure then
            fail SiloStage "ConfigureFunctionalPersistence requires a configure action."
    ...
    /// <summary>Use F#-aware JSON as both functional durable payload defaults.</summary>
    [<Extension>]
    static member UseFunctionalFSharpJsonPersistence(builder: ISiloBuilder) : ISiloBuilder =
        FunctionalGrainSiloHostingExtensions.ConfigureFunctionalPersistence(
            builder,
            Action<FunctionalPersistenceOptions>(fun options ->
                options.DefaultStateCodec <- FunctionalPersistenceCodec.FSharpJson
                options.DefaultJournalCodec <- FunctionalPersistenceCodec.FSharpJson)
        )
