...
| 12 | Stateless workers and flexible placement | **composed** | `Placement.fs`, tour §10. Orleans resolves placement from attributes on the grain *class*, and a functional grain's class is the library's own `FunctionalGrainMarker<'Actor>` — you cannot attribute it. But every placement attribute implements `IGrainPropertiesProviderAttribute`, and an application may register its own `IGrainPropertiesProvider`, so the attribute is applied to the grain type **by name**. Measured: 8 concurrent 400 ms calls to one grain id finish in ~0.8 s across 4 activations, against 1 activation and ~3.2 s without it. A typed `placement` / `statelessWorker` contract operation is still worth having; the capability is not blocked on it. |
