...
Nothing exercised by the tour is asserted from documentation: every exercised section prints
its own status line. Rows marked as not exercised here, or as integration-fixture evidence,
name that evidence explicitly.
...
|---|---|---|---|
| 1 | Persistence: `stateFrom` + extra `usePersistentState` holders | **supported** | `Persistence.fs`, tour §1. One primary holder whose loaded value *is* the handler's `state`, plus a second, differently-typed holder on a different provider. The primary descriptor also demonstrates `PersistentState.withSchema` with a typed version-0-to-1 upcaster. Every read, write and clear is explicit — returning new state from a handler publishes it in memory only. |
...
| 6 | Contract versioning, and version tolerance | **supported** | `VersioningTour.fs`, tour §6. Contract versions are native Orleans interface versions and are also validated in the functional envelope. Exact matching is the default; `acceptsVersions` widens host payload admission while Orleans' own compatibility/selector controls routing. A separate integration harness starts real N/N+1 executables and verifies rollback. |
| 7 | Streams | **supported** | `Streams.fs`, tour §7. The producer resolves the named `IStreamProvider` as a keyed service and demonstrates both single-item and provider-native batch publishing. The grain-side consumer uses `StreamHandlers` with item/error/completion callbacks. External-client and in-silo subscriptions receive every item; filtering, batch observers, terminal provider behavior, and token resumes are covered by live integration tests. |
...
| 11 | Implicit **stream** subscriptions for a functional grain | **supported** | `Implicit.fs`, tour §11. `onStream provider ns hook` and `onBroadcast provider ns hook` are first-class `grainFor` operations. The tour publishes to a stream key nothing has ever called, and the grain that key names is **activated by the publish** and runs the hook — `activations: 1`, `mail=[first; second]`, and a publish to an undeclared namespace delivering `0`. Broadcast channels ride the same machinery, so a functional grain no longer needs the class-grain consumer row 9 describes. What used to be here is kept as history in ["Implicit stream subscriptions: the wall, and how it was closed"](#implicit-stream-subscriptions-the-wall-and-how-it-was-closed). |
| 12 | Stateless workers, placement, and live migration | **supported** | `Placement.fs`, tour §10 demonstrates `statelessWorker 4`. The definition API also covers all six stock non-stateless strategies, including `HashBased` and `SiloRoleBased`. Successful `context.migrateOnIdle` moves plus repeatable `migrationParticipant` factories preserve ephemeral and application-owned activation state; a dedicated two-silo integration fixture verifies the actual transfer and placement manifests. |
| 13 | Distributed ACID transactions | **supported** | `Transactions.fs`, tour §14. `transactionalStateFrom (TransactionalState.create<'S> name storage)` attaches an Orleans transactional facet, and `transactional TransactionOption.X (_.op)` declares the per-operation policy. `tour.teller` creates one transaction and drives two `tour.account` participants inside it: a transfer of 40 leaves named balances `60` and `40` read back inside a single transaction, and a transfer of 500 aborts with `OrleansTransactionAbortedException` leaving the same snapshot — the earlier deposit rolled back with it. The overdraft rule itself is a pure `Result`; only the actor boundary translates its typed rejection into the fault Orleans needs for rollback. The tour also prints the two refusals the runtime adds (`readOnly` + `transactional` cannot update; an operation with no `transactional` declaration cannot touch the facet at all) and the measured re-execution answer: the aborted transfer entered the `withdraw` handler exactly **once**. The classic KEEP-path's definitions are unchanged, still registered and still under test in [`examples/bank-transactions`](../bank-transactions), whose live demo is now that example's own functional twin. |
...
| 16 | Event sourcing: `journaledGrainFor` | **supported** | `EventSourcing.fs`, tour §15. `initialEventState` seeds the fold, `apply` is the pure replay fold, and handlers return events which are confirmed before the reply. The example now declares independent typed `stateSchema` and `eventSchema` upcasters, and drives both built-in providers. CustomStorage adds configurable snapshots; built-in `LogStorage` still does not compact and `StateStorage` keeps only the folded view. |
...
**A functional grain can now consume a broadcast channel directly** — declare
