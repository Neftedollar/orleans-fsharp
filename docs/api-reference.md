...
| `contract<'Key, 'Api>` | `GrainContractBuilder<'Api,'Key,'Api>` — a value, not a function | Short form: the API record is its own actor brand ([details](functional-grains.md#the-short-form-the-api-record-as-its-own-brand)) |
...
spelled curried fails contract construction. See [Functional grains](functional-grains.md),
...
only to definitions with `customStorage`. See [Event Sourcing](event-sourcing.md#custom-storage-and-snapshots).
...
does not retry with JSON after a serialization exception. See [Serialization](serialization.md#explicit-generalized-serializers).
