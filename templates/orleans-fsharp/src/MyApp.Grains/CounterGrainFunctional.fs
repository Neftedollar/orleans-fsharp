/// <summary>
/// Functional-runtime equivalent of <c>CounterGrainDef.counter</c> in <c>CounterGrain.fs</c>
/// (the <c>grain { }</c> CE original -- now <c>[&lt;Obsolete&gt;]</c>). Same domain
/// (increment / decrement / read a count), rebuilt as a <c>grainContract</c> + <c>grainFor</c>
/// pair. See docs/functional-grains.md in the Orleans.FSharp repo for the full migration guide.
/// </summary>
...
module CounterApi =
    let contract =
        grainContract<CounterActor, string, CounterApi> () {
            grainType "myapp.counter.functional"
            version 1
            stringKey
        }
